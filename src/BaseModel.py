    ...
    * interaction effects (functions of distance in time and space relative to each datapoint)
    ...
    def init_model(self, target):
        ...
        if self.include_ia:
            with pm.Model() as self.model:
                ...
                expanded_TT = np.reshape(T_T, newshape=(21, 412, 2)) # TODO
            ...
            with pm.Model() as self.model:
                ...
                expanded_Wtt = tt.tile(
                    W_t_t.reshape(shape=(1, num_counties, -1)), reps=(21, 1, 1) # TODO: 21 days hardcoded, letzte 1 ist linearität
                )
    ...
    def sample_predictions(
        ...
        expanded_Wtt = np.tile(
            np.reshape(W_t_t, newshape=(-1, 1, 412, 2)), reps=(1, 31, 1, 1)
        )
        expanded_TT = np.reshape(T_T, newshape=(1, 31, 412, 2))
        result_TT = np.reshape(
            np.sum(expanded_TT * expanded_Wtt, axis=-1), newshape=(-1, 31 * 412)
        )
